...
/*  Ovenfor preorder?    Nedenfor postorder?    Nei !!! - da if-else-grener.

void grener(Node* node) {
  if (node) {
     if (node->left  ||  node->right) {  //  Leter videre nedover etter bladnoder:
        grener(node->left);     //  Besøker venstre subtre.
        grener(node->right);    //  Besøker høyre subtre.
     } else {                   //  Bladnode - skriver stien ned til den:
        gren(node);  cout << '\n';
     }
  }
}
